  ...
  const [eventCoordinators, setEventCoordinators] = useState([]);
    ...
    useEffect(() => {
     ...
     function getSupportingLists(){
        ...
        axios.all([
          axios.get('/events/getcategories'),
          axios.get('/events/getpartnerships'),
          axios.get('/events/getaccountingcodes'),
         // axios.get('/events/getcoordinators')
        ])
        .then(responseArr => {
    ...
    const handleSubmit = async e => {
        ...
        axios.post(submitPath, bomEvent )
        .then(response => {
